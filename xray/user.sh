...
Name=$(curl -sS https://raw.githubusercontent.com/YakkoRasZ14/YakkoRasZ14/permission/main/ip | grep $MYIP | awk '{print $2}')
...
autoscriptvps=`cat /etc/xray/$user/$user-tls.json`
yokkoeddy=`cat /etc/xray/$user/$user-none.json`
...
export vmesslink1="vmess://$(echo $autoscriptvps | base64 -w 0)"
export vmesslink2="vmess://$(echo $yokkoeddy | base64 -w 0)"
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"  | tee -a /etc/log-create-user.log
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}" | tee -a /etc/log-create-user.log
echo -e "$COLOR1 ${NC}                ${WH}• BY YOKKO EDDY STORE •${NC}                 $COLOR1 $NC" | tee -a /etc/log-create-user.log
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" | tee -a /etc/log-create-user.log
