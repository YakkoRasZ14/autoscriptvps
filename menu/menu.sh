...
export COLOR1="$(cat /etc/autoscriptvps/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/autoscriptvps/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"
WH='\033[1;37m'
###########- END COLOR CODE -##########
...
Exp=$(curl -sS https://raw.githubusercontent.com/YakkoRasZ14/permission/main/ip | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
Exp1=$(curl https://raw.githubusercontent.com/YakkoRasZ14/permission/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/YakkoRasZ14/autoscriptvps/main/menu/install-up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/YakkoRasZ14/autoscriptvps/main/menu/versi )
...
echo -e "$COLOR1 ${NC}             ${WH}• WA : 081949400586 •${NC}             $COLOR1 $NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e ""
