...
TIMES="10"
CHATID=`cat /etc/bot/id`
KEY=`cat /etc/bot/token`
...
<code>Host Slowdns     : $sldomain</code>
<code>Pub Key          : $slkey</code>
<code>Port OpenSSH     : $opensh</code>
<code>Port Dropbear    : $db</code>
<code>Port DNS         : 443,53</code> 
<code>Port SSH WS      : 80, 8080</code>
<code>Port SSH SSL WS  : $wsssl</code>
<code>Port SSL/TLS     : 8443,8880</code>
<code>Port OVPN WS SSL : 2086</code>
<code>Port OVPN SSL    : 990</code>
<code>Port OVPN TCP    : $ovpn</code>
<code>Port OVPN UDP    : $ovpn2</code>
...
<code>───────────────────────────</code>
<code>OpenVPN WS SSL   : </code>https://$domain:89/$domain-ws-ssl.ovpn
<code>OpenVPN SSL      : </code>https://$domain:89/$domain-ssl.ovpn
<code>OpenVPN TCP      : </code>https://$domain:89/$domain-tcp.ovpn
<code>OpenVPN UDP      : </code>https://$domain:89/$domain-udp.ovpn
<code>───────────────────────────</code>
...
echo -e "$COLOR1 ${NC}                ${WH}• YakkoRasZ14 •${NC}                 $COLOR1 $NC" | tee -a /etc/log-create-user.log
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" | tee -a /etc/log-create-user.log
else
